...
import { AddressActivity, AddressActivityBodyType, Directory, ReactionType } from '../src/api';
...
describe('signature verification', function() {
  describe("#isValidDirectorySignature()", function() {
    ...
    it('should accept a known valid signature', async function() {
      expect(await Farcaster.isValidDirectorySignature(address, correctlySignedDirectory)).to.be.true;
    });

    it('should reject an invalid signature', async function() {
      const withInvalidSignature = { ...correctlySignedDirectory, signature: '0xda120282bf3ff6ffff7b079c27a9479d5ec7cf31e270d9700c1f1e2c0db5583b1a04ea324b6ff4bb6551b88b5176271f5db60727be63b434b0b5c31565a1e12d1c' };
      expect(await Farcaster.isValidDirectorySignature(address, withInvalidSignature)).to.be.false;
    });

    it('should reject a valid signature with an incorrect merkle root', async function() {
      const withInvalidMerkleRoot = { ...correctlySignedDirectory, merkleRoot: '0x2ef0c4ce20ce44d86b06418509135ac5a15b14e4a569bbd002d276c4596706ac' };
      expect(await Farcaster.isValidDirectorySignature(address, withInvalidMerkleRoot)).to.be.false;
    });
  });
  ...
  describe('#isValidAddressActivitySignature()', function() {
    ...
    it('should accept a known valid signature', async function() {
      expect(await Farcaster.isValidAddressActivitySignature(address, correctlySignedActivity)).to.be.true;
    });

    it('should reject an invalid signature', async function() {
      const withInvalidSignature = { ...correctlySignedActivity, signature: '0xfe487ba982022b39310708fc95af654ca45b708d72e3e4dc02d07fed904859920d7f4ff27ad8ac4a833e3df98ddfe5d4375a067f21a0462a37030122b0c637551c' };
      expect(await Farcaster.isValidAddressActivitySignature(address, withInvalidSignature)).to.be.false;
    });

    it('should reject a valid signature with an incorrect merkle root', async function() {
      const withInvalidMerkleRoot = { ...correctlySignedActivity, merkleRoot: '0xb1661819a968f0bd4ea727add2d49f4799b43a5dd189ec7004de91263079577d' };
      expect(await Farcaster.isValidAddressActivitySignature(address, withInvalidMerkleRoot)).to.be.false;
    });
  });
});
