...
export class FarcasterGuardianContentHost implements ContentHost {
    ...
    constructor(privateKey: string | Uint8Array, network: Networkish = 'rinkeby', axiosInstance?: AxiosInstance) {
        ...
        this.axiosInstance.interceptors.request.use(this._axiosIntercepter.bind(this));
    }
    ...
    async updateDirectory(address: string, newDirectory: Directory): Promise<void> {
        return this.axiosInstance.post(`/indexer/directory/${address}`, { 'directory': newDirectory });
    }

    private async _axiosIntercepter(request: AxiosRequestConfig): Promise<AxiosRequestConfig> {
        const timeinEpochSeconds = Math.floor(Date.now() / 1000);
        const expiry = timeinEpochSeconds + 60;
        const jwt = await createJWT(
            { exp: expiry },
            { issuer: `did:ethr:${this.canonicalNetwork.name}:${this.address}`, signer: this.jwtSigner },
            { alg: 'ES256K' }
        );
        request.headers = {
            ...request.headers,
